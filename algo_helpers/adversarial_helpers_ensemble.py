...
from algo_helpers.algo_helpers import LLMModel, EnsembleModel, EvaluationConfig, extract_json, ResponseEvaluationTensor, parse_args
...
class AdversarialEvaluation (ResponseEvaluationTensor):
    ...
    def __init__(self, models_config: str):
        ...
        self.most_recent_ensemble = ""

        if "ensemble" in config:
            ensemble_models = config["ensemble"]["models"]
            self.test_models.append(EnsembleModel(models=[ensemble_models]))
        ...
        def process_evaluator():
            ...
            for trial in range(config.num_trials):
                ...
                for col_idx in range(len(self.test_models)):
                    ...
                    if isinstance(model_under_test, EnsembleModel):
