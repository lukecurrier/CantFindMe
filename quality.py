...
def plot_metrics(model_name: str, results: dict, index: int):
    ...
    # Apply a colormap based on the normalized perplexity values
...
def plot_text_diversity(models, ttr_values):
    ...
    """
    # Check if the number of models and TTR values match
    if len(models) != len(ttr_values):
        raise ValueError("The number of models must match the number of TTR value lists.")
